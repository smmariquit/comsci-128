...
async function restore() {
  ...
  const { data: users, error: uErr } = await supabase
    ...
    .in("account_email", [
      "ygprudencio@up.edu.ph",
      "peter.centeno@gmail.com",
      "jagarcia@up.edu.ph",
    ]);
  ...
  console.log(
    "Re-inserting 5 default housings with original IDs and coordinates...",
  );
  ...
  const { error: roomInsErr } = await supabase
    .from("room")
  ...
  if (mrhRoomInsErr) {
    ...
    console.log(
      `Successfully inserted ${mrhRooms.length} MRH detailed rooms with coordinate mapping!`,
    );
  }
