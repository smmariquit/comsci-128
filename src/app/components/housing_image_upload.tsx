...
export default function HousingImageUpload() {
  ...
  const handleUpload = async () => {
    ...
    if (!housingIdInput || isNaN(numericId)) {
      toast.error("Please enter a valid numeric Housing ID.");
      return;
    }
    ...
    try {
      ...
      toast.success(`Success! Image uploaded for Housing ID: ${numericId}`);
      ...
      toast.error("Failed to upload image. Please check the console.");
    } finally {
        ...
        <div>
          ...
          >
            {previewUrl ? (
              ...
              >
                {/* Decorative Icon */}
                <ImageIcon size={24} color={C.teal} strokeWidth={2} />
