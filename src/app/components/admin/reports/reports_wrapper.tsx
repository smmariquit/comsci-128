...
import {
  StatCard,
  ExportButton,
} from "@/app/components/admin/reports/reportsui";
...
export default function ReportsWrapper({
  ...
  const filteredAccommodation = useMemo(
    () =>
      liveAccommodationHistory.filter((r) => {
        const q = search.toLowerCase();
  ...
  function OccupancyStats() {
    ...
    const fully_occupied = d.filter(
      (r) => r.occupancy_status === "Fully Occupied",
    ).length;
    const partial_occupied = d.filter(
      (r) => r.occupancy_status === "Partially Occupied",
    ).length;
  ...
  function ApplicationStats() {
    ...
    const approved = d.filter(
      (r) => r.application_status === "Approved",
    ).length;
    const pending = d.filter(
      (r) =>
        r.application_status === "Pending Admin Approval" ||
        "Pending Manager Approval",
    ).length;
    const rejected = d.filter(
      (r) => r.application_status === "Rejected",
    ).length;
    return (
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
  ...
  function AccommodationStats() {
      ...
      d.length > 0
        ? Math.round(
            d.reduce((s, r) => {
              return (
                s +
                (new Date(r.moveout_date).getTime() -
                  new Date(r.movein_date).getTime()) /
                  (1000 * 60 * 60 * 24)
              );
            }, 0) / d.length,
          )
    ...
    return (
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
        ...
        <StatCard
          label="Avg Stay"
          value={`${Math.round(avgDays / 30)}mo`}
          deltaSub="per record"
        />
      </div>
    );
  ...
  function handleExportCSV() {
    console.log("Export CSV:", activeTab); /* TODO */
  }
  function handleExportPDF() {
    console.log("Export PDF:", activeTab); /* TODO */
  }
