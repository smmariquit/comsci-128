...
function Divider() {
  return (
    <div style={{ height: 1, background: C.dividerLight, margin: "4px 0" }} />
  );
}
...
function OccupancyDetail({ data }: { data: OccupancyReportRow }) {
  const rate =
    data.maximum_occupants > 0
      ? Math.round(
          ((data.maximum_occupants - data.current_occupants) /
            data.maximum_occupants) *
            100,
        )
      : 0;
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
      <Field label="Room Code">
        <span style={{ fontFamily: "monospace" }}>{data.room_code}</span>
      </Field>
      <Field label="Property">{data.housing_name}</Field>
      <Field label="Room Type">
        <RoomTypeTag type={data.room_type} />
      </Field>
      <Field label="Status">
        <OccupancyBadge status={data.occupancy_status} />
      </Field>
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
      <Field label="Application ID">
        <span style={{ fontFamily: "monospace" }}>#{data.application_id}</span>
      </Field>
      <Field label="Status">
      ...
      <Field label="Student No.">
        <span style={{ fontFamily: "monospace" }}>{data.student_number}</span>
      </Field>
      <Field label="Property">{data.housing_name}</Field>
      <Field label="Preferred Room">
        <RoomTypeTag type={data.preferred_room_type} />
      </Field>
      <Field label="Expected Move-out">
        {formatDate(data.expected_moveout_date)}
      </Field>
      <Field label="Actual Move-out">
        {data.actual_moveout_date ? (
          <span style={{ color: C.orange, fontWeight: 600 }}>
            {formatDate(data.actual_moveout_date)}
          </span>
        ) : (
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
      ...
      <Field label="Bill Type">
        <BillTypeTag type={data.bill_type} />
      </Field>
      <Field label="Amount">
        <span
          style={{
      ...
      <Field label="Due Date">
        <span
          style={{
            color: data.status === "Overdue" ? C.orange : C.navy,
            fontWeight: data.status === "Overdue" ? 700 : 500,
          }}
        >
...
function AccommodationDetail({ data }: { data: AccommodationHistoryRow }) {
  const days = Math.round(
    (new Date(data.moveout_date).getTime() -
      new Date(data.movein_date).getTime()) /
      (1000 * 60 * 60 * 24),
  );
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
      ...
      <Field label="Duration">
        <span
          style={{ fontFamily: "monospace", color: C.teal, fontWeight: 600 }}
        >
          ~{months} months
        </span>
      </Field>
    ...
    <>
      ...
      >
        ...
        <div style={{ padding: "20px" }}>
          ...
          {row.kind === "accommodation" && (
            <AccommodationDetail data={row.data} />
          )}
        </div>
