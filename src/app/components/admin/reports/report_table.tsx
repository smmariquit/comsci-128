...
export function OccupancyReportTable({ data, onView }: OccupancyProps) {
  ...
  const vacancyRate =
    data.length > 0
      ? Math.round(
          (data.filter((r) => r.occupancy_status === "Empty").length /
            data.length) *
            100,
        )
      : 0;
    ...
    <TableShell title="Occupancy Report" count={data.length}>
      ...
      <tbody>
        {data.length === 0 ? (
          ...
          data.map((row, i) => {
            const rate =
              row.maximum_occupants > 0
                ? Math.round(
                    ((row.maximum_occupants - row.current_occupants) /
                      row.maximum_occupants) *
                      100,
                  )
                : 0;
            return (
        ...
        {data.length === 0 ? (
          ...
          data.map((row, i) => (
            ...
            >
              ...
              <Td>
                <RoomTypeTag type={row.preferred_room_type} />
              </Td>
              <Td>
...
export function AccommodationHistoryTable({
  data,
  onView,
}: AccommodationProps) {
  const [hoveredRow, setHoveredRow] = useState<number | null>(null);
        ...
        {data.length === 0 ? (
          ...
          data.map((row, i) => {
            const days = Math.round(
              (new Date(row.moveout_date).getTime() -
                new Date(row.movein_date).getTime()) /
                (1000 * 60 * 60 * 24),
            );
            const months = Math.round(days / 30);
            return (
              <tr
                key={`${row.account_number}-${row.room_id}`}
                onMouseEnter={() => setHoveredRow(i)}
                onMouseLeave={() => setHoveredRow(null)}
                style={{
                  borderTop: i === 0 ? "none" : `1px solid ${C.dividerLight}`,
