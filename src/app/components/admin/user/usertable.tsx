// src/app/components/admin/user/usertable.tsx
...
const HOUSING_STATUS_STYLE: Record<
  HousingStatus,
...
function AccountStatusBadge({ isDeleted }: { isDeleted: boolean }) {
  const label = isDeleted ? "Removed" : "Active";
  const color = isDeleted ? C.orange : C.teal;
  const bg = isDeleted ? "rgba(201,100,42,0.10)" : "rgba(86,115,117,0.12)";

  return <Badge label={label} bg={bg} text={color} dot={color} />;
}
