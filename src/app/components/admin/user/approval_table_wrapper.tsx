...
> = {
  ...
  Approved: {
    bg: "rgba(86,115,117,0.12)",
    dot: C.teal,
    text: C.teal,
    label: "Approved",
  },
  Rejected: {
    bg: "rgba(201,100,42,0.10)",
    dot: C.orange,
    text: C.orange,
    label: "Rejected",
  },
  Cancelled: {
    bg: "rgba(28,38,50,0.08)",
    dot: C.navy,
    text: C.navy,
    label: "Cancelled",
  },
...
const BTN_STYLE: Record<BtnVariant, React.CSSProperties> = {
  ...
  approve: {
    background: "rgba(86,115,117,0.12)",
    color: C.teal,
    border: "1px solid rgba(86,115,117,0.25)",
  },
};
                  ...
                  <div style={{ display: "flex", gap: 4 }}>
                    ...
                    {isPending(row) && (
                      <>
                        <ActionBtn
                          label="Approve"
                          onClick={() => onApproveInit(row)}
                          variant="approve"
                        />
                        <ActionBtn
                          label="Reject"
                          onClick={() => onRejectInit(row)}
                          variant="danger"
                        />
                      </>
                    )}
...
function ConfirmModal({
      ...
      const fetchRooms = async () => {
        ...
        try {
          const res = await fetch(
            `/api/rooms/available?housingId=${config.row.housing_id}&roomType=${config.row.preferred_room_type}`,
          );
  ...
  const finalRoomToSubmit = overrideRoomId
    ? Number(overrideRoomId)
    : config.row.room_id;

  const tentativeRoomDisplay = config.row.room_code
    ? `Room ${config.row.room_code}`
    : config.row.room_id
      ? `Room ID: ${config.row.room_id}`
      : "None Assigned";
    ...
    >
      ...
      >
        ...
        >
          ...
          <strong>{isApprove ? "approve" : "reject"}</strong> the housing
          application for{" "}
          <span style={{ color: C.teal, fontWeight: 500 }}>
            {config.row.student_name}
          </span>
          ?
        </p>
          ...
          >
            ...
            <div style={{ marginBottom: 16 }}>
              ...
              <div style={{ fontSize: 14, color: C.navy, fontWeight: 500 }}>
                ...
                >
                  ({config.row.preferred_room_type})
                </span>
              </div>
            </div>
...
function ViewModal({
  ...
  const detailRows: Array<{ label: string; value: React.ReactNode }> = [
    ...
    { label: "Housing", value: row.housing_name },
    {
      ...
      value: row.preferred_room_type ?? (
        <span style={{ opacity: 0.45 }}>—</span>
      ),
    },
    {
      label: "Expected Move-Out",
      value: row.expected_moveout_date ? (
        new Date(row.expected_moveout_date).toLocaleDateString()
      ) : (
        <span style={{ opacity: 0.45 }}>—</span>
      ),
    },
    {
      ...
      value: row.actual_moveout_date ? (
        new Date(row.actual_moveout_date).toLocaleDateString()
      ) : (
        <span style={{ opacity: 0.45 }}>—</span>
      ),
    },
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <h3
            style={{ margin: 0, fontSize: 16, fontWeight: 600, color: C.navy }}
          >
            Application Details
          </h3>
          <StatusBadge status={row.application_status} />
...
export default function ApplicationTable_Wrapper({
  ...
  const [applications, setApplications] =
    useState<ApplicationReportRow[]>(liveApplications);
  ...
  const patchApplicationStatus = async (
    ...
    if (!response.ok) {
      ...
      throw new Error(
        payload?.message ?? "Failed to update application status.",
      );
    }
  };
  ...
  const handleConfirmAction = async (finalRoomId?: number) => {
    ...
    try {
      if (modalConfig.action === "approve") {
        if (!finalRoomId)
          throw new Error("Cannot approve: No room has been assigned.");

        const response = await fetch(
          `/api/applications/${modalConfig.row.application_id}/assign`,
          {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
        ...
        await patchApplicationStatus(
          modalConfig.row.application_id,
          "Rejected",
        );
  ...
  const counts = useMemo(() => {
    const c = { All: applications.length } as Record<
