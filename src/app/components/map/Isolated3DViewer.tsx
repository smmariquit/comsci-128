...
export default function Isolated3DViewer({
  housing,
  onClose,
}: Isolated3DViewerProps) {
  ...
  useEffect(() => {
    ...
    map.on("load", () => {
      ...
      if (map.getSource(buildingSourceId) && !map.getLayer("3d-buildings")) {
        ...
        console.warn(
          `Missing map source: ${buildingSourceId}. Ensure /map-style.json defines it.`,
        );
      }
      ...
      if (housing.rooms && housing.rooms.length > 0) {
        housing.rooms.forEach((room) => {
          if (room.latitude && room.longitude) {
            ...
            el.className =
              "px-1.5 py-0.5 bg-white text-black text-[9px] font-bold rounded shadow-sm border border-gray-300 whitespace-nowrap z-10 hover:bg-orange-100 hover:border-orange-400 cursor-pointer transition-colors";
      ...
      <div className="relative w-full max-w-5xl h-[80vh] bg-white rounded-xl overflow-hidden shadow-2xl border border-gray-200 flex">
        ...
        <div className="w-64 bg-[#f8f9fa] border-r border-gray-200 p-4 overflow-y-auto flex flex-col z-10">
          ...
          <p className="text-xs text-gray-500 mb-6">
            3D model fabricated from OpenStreetMap footprint
          </p>
          ...
          <div className="mb-4">
            ...
            <div className="flex flex-wrap gap-2">
              <span className="px-2 py-1 bg-white border border-gray-300 rounded text-xs text-gray-700 cursor-pointer hover:bg-gray-50">
                All floors
              </span>
              <span className="px-2 py-1 bg-[#8c3123] text-white rounded text-xs cursor-pointer shadow-inner">
                Floor 1
              </span>
              <span className="px-2 py-1 bg-white border border-gray-300 rounded text-xs text-gray-700 cursor-pointer hover:bg-gray-50">
                Floor 2
          ...
          <div>
            ...
            <div className="space-y-1">
              {housing.rooms &&
                housing.rooms.map((room, i) => (
                  <div
                    key={room.room_id || i}
                    className="flex justify-between items-center py-2 border-b border-gray-100 hover:bg-gray-100 px-2 cursor-pointer rounded"
                  >
