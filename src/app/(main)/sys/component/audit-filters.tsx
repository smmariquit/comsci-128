...
export default function AuditFilters({
  ...
  const activeTags = [
    values.action !== "All Actions" && {
      key: "action",
      label: `Action: ${values.action}`,
      clear: () => set("action", "All Actions"),
    },
    values.module !== "All Modules" && {
      key: "module",
      label: `Module: ${values.module}`,
      clear: () => set("module", "All Modules"),
    },
    ...
    <div className="flex flex-col gap-3">
      ...
      {activeTags.length > 0 && (
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-xs text-[#1a2332]/40 font-medium">
            Filters:
          </span>
          {activeTags.map((tag) => (
            <span
