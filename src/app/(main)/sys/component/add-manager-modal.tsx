          ...
          <div className="overflow-y-auto max-h-[90vh]">
            ...
            <div className="flex items-start justify-between px-8 py-6 border-b border-[#1a2332]/8">
              <div>
                ...
                <p className="text-sm text-[#1a2332]/40 font-mono mt-0.5">
                  Assign a manager to a dormitory
                </p>
              </div>
              <button
                onClick={onClose}
              ...
              <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="flex gap-3 h-full items-center px-4 py-3">
                    {(["Male", "Female"] as const).map((opt) => (
                      ...
                      >
                        ...
                        >
                          {form.sex === opt && (
                            <div className="w-2 h-2 rounded-full bg-[#d4622a]" />
                          )}
                        </div>
                        {opt}
                      </button>
                ...
                <Field
                  label={
                    <>
                      Contact Number{" "}
                      <span className="text-[#1a2332]/35 font-normal italic">
                        [optional]
                      </span>
                    </>
                  }
            ...
            <div className="px-8 py-6 flex flex-col gap-6">
              ...
              <Field
                label="Email"
                placeholder="e.g. llfernandez4@up.edu.ph"
                value={form.email}
              ...
              {/* Role picker */}
              <div>
                <p className="text-sm font-semibold text-[#1a2332] mb-3">
                  Role
                </p>
                <div className="grid grid-cols-2 gap-3">
                  <RoleCard
                    selected={form.role === "Manager"}
                    onClick={() => set("role", "Manager")}
                    title="Housing Administrator"
                    description="Manage dorm and tenants"
                  />
                  <RoleCard
                    selected={form.role === "Landlord"}
                    onClick={() => set("role", "Landlord")}
                    title="Landlord"
                    description="Owner-level property access"
                  />
                </div>
              </div>
              ...
              {/* Assign to Dormitory */}
              <div>
                <p className="text-[13px] font-semibold text-[#d4622a] mb-2">
                  Assign to Dormitory
                </p>
                <select
                  value={form.dorm}
                ...
                <p className="text-xs text-[#1a2332]/40 mt-1.5">
                  The manager will only have access to the selected dormitory
                </p>
              </div>
...
function RoleCard({
  ...
  return (
    ...
    >
      ...
      <div>
        <p
          className={`text-sm font-bold ${selected ? "text-[#1a2332]" : "text-[#1a2332]/60"}`}
        >
          {title}
        </p>
