...
function ActionPill({ action }: { action: ActionType }) {
  const style =
    ...
    "bg-[#f3f4f5] text-[#1a2332]/60 ring-1 ring-gray-200";
  const dot = ACTION_DOT[action] ?? "bg-gray-400";
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold ${style}`}
    >
          ...
          <div className="flex-1 overflow-y-auto px-6 py-5 space-y-5">
            ...
            <div className="flex items-center justify-between gap-3 p-4 bg-[#f3f4f5] rounded-xl border border-gray-100">
              <div className="flex flex-col gap-2">
                ...
                <p className="text-[11px] text-[#1a2332]/40 font-mono pl-1">
                  {log.module} module
                </p>
              </div>
            ...
            <div className="divide-y divide-gray-100 rounded-xl border border-gray-100 overflow-hidden bg-white">
              {/* Timestamp */}
              <DetailRow
                icon={<Clock size={14} className="text-[#1a2332]/50" />}
                label="Timestamp"
              >
                <span className="font-bold text-[#1a2332]">
                  {formatTime(log.timestamp)}
                </span>
                <span className="text-[#1a2332]/40 text-xs font-mono ml-2">
              ...
              {/* User */}
              <DetailRow
                icon={<User size={14} className="text-[#1a2332]/50" />}
                label="User"
              >
                <div className="flex items-center gap-2.5">
                  <div
                    className={`w-7 h-7 rounded-full ${log.userColor ?? "bg-[#1a2332]"} flex items-center justify-center shrink-0`}
                  >
              ...
              {/* Action */}
              <DetailRow
                icon={<Activity size={14} className="text-[#1a2332]/50" />}
                label="Action"
              >
                {log.action}
              </DetailRow>
              ...
              {/* Module */}
              <DetailRow
                icon={<Tag size={14} className="text-[#1a2332]/50" />}
                label="Module"
              >
              ...
              {/* IP Address */}
              <DetailRow
                icon={<Globe size={14} className="text-[#1a2332]/50" />}
                label="IP Address"
              >
              ...
              {/* Status */}
              <DetailRow
                icon={<Monitor size={14} className="text-[#1a2332]/50" />}
                label="Status"
              >
                <StatusBadge status={log.status} />
              </DetailRow>
            ...
            <div className="flex items-center justify-between px-4 py-2.5 bg-[#f3f4f5] rounded-xl border border-gray-100">
              <span className="text-[10px] font-semibold uppercase tracking-wider text-[#1a2332]/30">
                Log ID
              </span>
              <span className="text-[10px] font-mono text-[#1a2332]/40">
                #{log.id}
              </span>
            </div>
          </div>
