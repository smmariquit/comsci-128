      ...
      <div className="flex-1 flex flex-col overflow-auto">
        ...
        <div className="grid grid-cols-4 gap-4 px-8 mb-6">
          {stats.map((s) => (
            ...
            >
              <p
                className={`text-[10px] font-semibold tracking-widest uppercase mb-2 ${s.dark ? "text-white/50" : "text-[#1a2332]/40"}`}
              >
                {s.label}
              </p>
        ...
        <div className="flex gap-4 px-8 pb-8">
          ...
          <div className="flex-1 flex flex-col gap-4 min-w-0">
            ...
            <div className="bg-white rounded-2xl p-6">
              ...
              <div className="flex items-center justify-between mb-5">
                ...
                <Link
                  href="/sys/logs"
                  className="text-xs text-[#1a2332]/50 hover:text-[#d4622a] transition-colors flex items-center gap-1"
                >
              ...
              <div className="flex flex-col divide-y divide-[#1a2332]/6">
                {recentActivity.length === 0 ? (
                  <p className="text-xs text-[#1a2332]/40 py-4 text-center">
                    No recent activity.
                  </p>
                ) : (
                  recentActivity.map((item, i) => (
                    <div key={i} className="flex items-center gap-3 py-3">
                      ...
                      />{" "}
                      {/*ui circle color based on activity type*/}
                      <div className="flex-1 min-w-0">
                        <p className="text-sm text-[#1a2332] font-medium truncate">
                          {item.text}
                        </p>
                        <p className="text-[11px] text-[#1a2332]/40 mt-0.5">
                          {item.meta}
                        </p>
                      </div>
                      <span className="text-[11px] text-[#1a2332]/40 shrink-0">
                        {item.time}
                      </span>
                    </div>
                  ))
              ...
              <div className="flex flex-col gap-4">
                {occupancy.map((d) => (
                  <div key={d.name} className="flex items-center gap-4">
                    <span className="text-sm text-[#1a2332] font-medium w-14 shrink-0">
                      {d.name}
                    </span>
                    ...
                    <span
                      className={`text-sm font-semibold w-10 text-right shrink-0 ${d.pct >= 90 ? "text-[#d4622a]" : "text-[#1a2332]"}`}
                    >
                      {d.pct}%
                    </span>
                  </div>
          ...
          <div className="w-65 flex flex-col gap-4 shrink-0">
            <div className="bg-white rounded-2xl p-6">
              <h2 className="text-[15px] font-bold text-[#1a2332] mb-4">
                Quick Access
              </h2>
                  ...
                  href ? (
                    ...
                    >
                      <Icon
                        size={16}
                        strokeWidth={1.8}
                        className="text-[#1a2332]/50 group-hover:text-white transition-colors"
                      />
                      {label}
                    </Link>
                  ) : (
