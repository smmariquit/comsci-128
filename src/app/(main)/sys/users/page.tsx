  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        const [response, dormResponse, landlordResponse, housingAdminResponse] =
          await Promise.all([
            fetch("/api/users"),
            fetch("/api/housing"),
            fetch("/api/landlord"),
        ...
        const rawLandlords = Array.isArray(landlordData)
          ? landlordData
          : (landlordData.data ?? []);
        const rawHousingAdmins = Array.isArray(housingAdminData)
          ? housingAdminData
          : (housingAdminData.data?.data ?? []);

        const landlordIds = new Set(
          rawLandlords.map((l: any) => String(l.account_number)),
        );
        const housingAdminIds = new Set(
          rawHousingAdmins.map((m: any) => String(m.account_number)),
        );
  ...
  const filtered =
    users && Array.isArray(users)
      ? users.filter((u) => {
          ...
          const matchRole =
            filters.role === "All Roles" || u.role === filters.role;
          const matchStatus =
            filters.status === "All Status" || u.status === filters.status;
          const matchDorm =
            filters.dorm === "All Dorm" || u.dormitory === filters.dorm;
          const matchSex =
            filters.sex === "All Sex" || u.gender === filters.sex;
          return (
            matchSearch && matchRole && matchStatus && matchDorm && matchSex
          );
        })
      : [];
          ...
          <div className="bg-white rounded-2xl overflow-hidden">
            <div className="flex items-center justify-between px-6 py-4 border-b border-[#1a2332]/6">
              ...
              <span className="text-xs text-[#1a2332]/40">
                Showing{" "}
                {sorted.length === 0 ? 0 : (page - 1) * ITEMS_PER_PAGE + 1}–
              ...
              <tbody className="divide-y divide-[#1a2332]/5">
                {paginated.length === 0 ? (
                  <tr>
                    <td
                      colSpan={7}
                      className="text-sm text-[#1a2332]/40 text-center py-12"
                    >
                      No users found.
                    </td>
                  </tr>
