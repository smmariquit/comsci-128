...
export default function DormManagementPage({
  ...
  const fetchDorms = async () => {
    try {
      ...
      if (!housingResponse.ok)
        throw new Error(`HTTP error! status: ${housingResponse.status}`);
      if (!occupancyResponse.ok)
        throw new Error(`HTTP error! status: ${occupancyResponse.status}`);
      if (!landlordResponse.ok)
        throw new Error(`HTTP error! status: ${landlordResponse.status}`);
      if (!managerResponse.ok)
        throw new Error(`HTTP error! status: ${managerResponse.status}`);
      ...
      const rawOccupancy = Array.isArray(occupancyData)
        ? occupancyData
        : (occupancyData.data ?? []);
      const occupancyMap = new Map(
        rawOccupancy.map((o: any) => [String(o.housingId), o]),
      );
      ...
      const managerMap = new Map(
        transformedManagers.map((m: any) => [m.id, m]),
      );

      // Use housing as base, map occupancy data onto it
      const rawHousing = Array.isArray(housingData)
        ? housingData
        : (housingData.data ?? []);

      // Transform landlords
      const rawLandlords = Array.isArray(landlordData)
        ? landlordData
        : (landlordData.data ?? []);
      const transformedLandlords: LandlordOption[] = rawLandlords.map(
        (l: any) => {
      ...
      const transformed: Dorm[] = rawHousing.map((housing: any) => {
        ...
        const manager = managerMap.get(managerId) as
          | { id: string; name: string; email: string }
          | undefined;
        ...
        return {
          ...
          housingType:
            (housing.housing_type as "UP Housing" | "Non-UP Housing") ||
            "UP Housing",
  ...
  const filtered = DormList.filter((u) => {
    ...
    const matchHousingType =
      ...
      u.housingType === filters.housingType;
              ...
              <div className="flex items-center gap-1">
                ...
                {getVisiblePages(page, totalPages).map((p, idx) =>
                  typeof p === "number" ? (
                    ...
                    <span
                      key={idx}
                      className="px-2 text-sm text-[#1a2332]/40 select-none"
                    >
                      ...
                    </span>
                  ),
...
function PageBtn({
  ...
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className={`w-8 h-8 rounded-lg text-sm font-medium flex items-center justify-center transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#d4622a] focus-visible:ring-offset-2 ${
        active
