...
} from "@/app/(main)/sys/component/audit-filters";
...
function ActionBadge({ action }: { action: ActionType }) {
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[11px] font-semibold ${ACTION_STYLES[action]}`}
    >
...
export default function AuditLogsPage({
  ...
  const filtered = auditLogs.filter((l) => {
    ...
    const matchAction =
      filters.action === "All Actions" || l.action === filters.action;
    const matchModule =
      filters.module === "All Modules" || l.module === filters.module;
    const matchStatus =
      filters.status === "All Status" || l.status === filters.status;
  ...
  const handleExportPDF = () => {
    ...
    exportToPDF("Audit Logs Report", "audit-logs", headers, rows);
  };
        ...
        <div className="flex items-start justify-between px-8 pt-8 pb-6 border-b border-[#1a2332]/6">
          <div>
            ...
            <p className="text-sm text-[#1a2332]/50 mt-1 font-mono">
              Track all system activity, changes, and access events
            </p>
          </div>
          <div className="flex items-center gap-3">
            <button
              ...
              type="button"
              className="px-3 py-2 rounded-md bg-[#1a2332] text-white text-sm font-semibold hover:bg-[#253448]"
            >
              Export CSV
            </button>
            <button
              ...
              type="button"
              className="px-3 py-2 rounded-md border border-[#1a2332]/15 text-sm font-semibold hover:bg-[#eae8e1]"
            >
              Export PDF
            </button>
        ...
        <div className="px-8 py-6 flex flex-col gap-5">
          ...
          <div className="bg-white rounded-2xl overflow-hidden">
            ...
            <div className="divide-y divide-[#1a2332]/5">
              {paginated.length === 0 ? (
                <p className="text-sm text-[#1a2332]/40 text-center py-14">
                ...
                paginated.map((log) => (
                  ...
                  >
                    {/* TIMESTAMP */}
                    <div>
                      <p className="text-[12px] font-semibold font-mono text-[#1a2332]">
                        {formatTime(log.timestamp)}
                      </p>
                      <p className="text-[11px] text-[#1a2332]/40 font-mono">
                    ...
                    <div className="flex items-center gap-2 min-w-0">
                      <div
                        className={`w-7 h-7 rounded-full ${log.userColor ?? "bg-[#1a2332]"} flex items-center justify-center text-white text-[9px] font-bold shrink-0`}
                      >
                        {log.userInitials}
                      </div>
                    ...
                    {/* IP ADDRESS */}
                    <span className="text-[11px] font-mono text-[#1a2332]/60">
                      {log.ipAddress}
                    </span>
