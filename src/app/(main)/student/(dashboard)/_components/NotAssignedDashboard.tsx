...
export default function NotAssignedDashboard(
  ...
  const isTransient = (() => {
    if (!application?.expected_moveout_date || !application?.created_at)
      return false;
          ...
          <div className="p-5 space-y-4 max-h-[220px] overflow-y-auto divide-y divide-[#e3d8c9]/40">
            {history.filter(
              ...
              <div className="rounded-xl border border-dashed border-[#d9d2c4] bg-[#f7f2e8] p-4 text-sm text-[#567375]">
                You have not submitted a housing application yet. Browse
                available housing to get started.
                <div className="mt-3">
                  <Link
                    href="/student/browse"
