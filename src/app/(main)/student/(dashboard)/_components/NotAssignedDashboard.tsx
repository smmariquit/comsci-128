export default function NotAssignedDashboard(userName: String, userHousingDetails: Array<Object>) {
    ...
    function status(step: Object) {
        if("isDone" in step && step.isDone) {
            return checkIcon;
        }
        else {
            return crossIcon;
    ...
    <div className="w-216 flex-1 bg-stone-200 rounded-2xl shadow-[0px_2px_4px_0px_rgba(0,0,0,0.25)] flex flex-col justify-start items-start overflow-hidden">
      ...
      <div className="self-stretch flex-1 px-24 py-4 inline-flex justify-between items-center overflow-hidden">
        <div className="p-2.5 inline-flex flex-col justify-center items-center gap-2.5 overflow-hidden">
          {status(userHousingDetails[0])}
          <div className={tStyle}>Profile<br/>Created</div>
        </div>
        <div className="p-2.5 inline-flex flex-col justify-center items-center gap-2.5 overflow-hidden">
          {status(userHousingDetails[1])}
          <div className={tStyle}>Application<br/>Submitted</div>
        </div>
        <div className="p-2.5 inline-flex flex-col justify-center items-center gap-2.5 overflow-hidden">
          {status(userHousingDetails[2])}
          <div className={tStyle}>Manager<br/>Review</div>
        </div>
        <div className="p-2.5 inline-flex flex-col justify-center items-center gap-2.5 overflow-hidden">
          {status(userHousingDetails[3])}
