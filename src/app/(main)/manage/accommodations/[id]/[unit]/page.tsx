...
import { getRoomById, Room, RoomType } from "@/app/lib/data/room";
...
export default function Page() {
  ...
  useEffect(() => {
    ...
    if (loading) return <div className="text-white text-center p-10">Loading Room...</div>;
          ...
          {isEditing ? (
              <div className="space-y-4">
                  ...
                  <div className="flex gap-2">
                      <button onClick={handleSave} className="bg-green-600 px-4 py-2 rounded font-bold flex-1">Save</button>
                      <button onClick={() => setIsEditing(false)} className="bg-gray-600 px-4 py-2 rounded flex-1">Cancel</button>
                  </div>
              </div>
          ) : (
              <div className="space-y-2">
                  {room ? (
                      <>
                          ...
                          <p><strong>Max Occupants:</strong> {room.maximum_occupants}</p>
                          <p><strong>Status:</strong> {room.occupancy_status}</p>
                          <button 
