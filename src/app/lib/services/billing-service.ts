...
import {
  sendBillAssignedEmail,
  sendBillStatusUpdatedEmail,
} from "./email-service";
      ...
      if (accountNumber) {
        ...
        (async () => {
          try {
            ...
            console.error(
              "Failed to fetch student details for payment email:",
              err,
            );
          }
        })();
...
const removeBill = async (txnId: number) => {
  try {
    ...
    const { data: existingBill, error: fetchError } =
      await billData.getById(txnId);
      ...
      if (accountNumber) {
        ...
        (async () => {
          try {
            ...
            console.error(
              "Failed to fetch student details for bill creation email:",
              err,
            );
          }
        })();
