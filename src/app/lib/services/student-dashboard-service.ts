...
export function getApplicationSteps(application: any) {
  return [
    ...
    { label: "Application Submitted", isDone: !!application },
    {
      ...
      isDone: application?.application_status === "Approved",
    },
  ...
  if (application) {
    ...
    if (status === "Approved") {
      ...
      notifications.push({
        ...
        message:
          "Unfortunately, your housing application was not approved at this time.",
        created_at: application.updated_at || new Date().toISOString(),
      });
    }
  }
...
export async function completePaymentProcess(
  ...
  const updated = await DataLayer.updateBillPaymentProof(
    transactionId,
    publicUrl,
  );
