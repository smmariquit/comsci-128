...
async function getAccommodationHistoryOfStudent(studentAccountNumber: number) {
  ...
  if (error)
    throw new Error(`getAccommodationHistoryOfStudent Error: ${error.message}`);
  return data;
}
...
const getBillingSummary = async (accountNumber: number) => {
  ...
  const total_outstanding =
    data
      ?.filter(
        (bill: any) => bill.status === "Pending" || bill.status === "Overdue",
      )
...
async function deleteByAccountNumber(
  accountNumber: number,
  ...
  try {
    ...
    if (academicError) {
      return {
        deleted: false,
        error: `Failed to delete academic record: ${academicError.message}`,
      };
    }
    ...
    if (studentError) {
      return {
        deleted: false,
        error: `Failed to delete student record: ${studentError.message}`,
      };
    }
