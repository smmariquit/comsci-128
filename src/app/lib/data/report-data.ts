...
async function getOccupancyReport(
  ...
  return (rooms || []).map((room: any) => {
    const housingObj = Array.isArray(room.housing)
      ? room.housing[0]
      : room.housing;
...
async function getApplicationReport(
  ...
  if (error)
    throw new Error("Failed to fetch application report: " + error.message);
...
async function getAccommodationHistoryReport(
  managedHousingIds: number[],
): Promise<AccommodationHistoryRow[]> {
  ...
  return (histories || []).map((history: any) => {
    const studentObj = Array.isArray(history.student)
      ? history.student[0]
      : history.student;
    const userObj = Array.isArray(studentObj?.user)
      ? studentObj.user[0]
      : studentObj?.user;
    const roomObj = Array.isArray(history.room)
      ? history.room[0]
      : history.room;
    const housingObj = Array.isArray(roomObj?.housing)
      ? roomObj.housing[0]
      : roomObj?.housing;
    ...
    const effectiveMoveoutDate =
      history.moveout_date || new Date().toISOString();
...
async function getRevenueReport(
  ...
  return (bills || [])
    .map((bill: any) => {
      const studentObj = Array.isArray(bill.student)
        ? bill.student[0]
        : bill.student;
      const userObj = Array.isArray(studentObj?.user)
        ? studentObj.user[0]
      ...
      const rawStatus = String(bill.status ?? "")
        .trim()
        .toLowerCase();
