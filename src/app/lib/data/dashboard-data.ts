...
export async function getHousingAdmingDashboardData(landlordId: number) {
  ...
  const filteredRooms = allRooms.filter((r) =>
    managedHousingIds.includes(r.housing_id),
  );
  const filteredApps = allApps.filter(
    (app) => app.landlord_account_number === landlordId,
  );
  ...
  const totalOccupied = filteredRooms.reduce(
    (sum, r) => sum + (r.current_occupants || 0),
    0,
  );
