...
export async function getHousingAdmingDashboardData() {
  ...
  const totalPendingApplication = allApps.filter(
  ...
  const totalOccupied = allRooms.filter(
  ...
  const studentStatusList = allStudents
    .map((u) => u.student?.housing_status)
    .filter(Boolean);
  const totalAssigned = studentStatusList.filter(
    (s) => s === "Assigned",
  ).length;
  const totalUnassigned = studentStatusList.filter(
    (s) => s === "Not Assigned" || s === "Vacated",
  ).length;
