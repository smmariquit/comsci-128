...
async function getUsersForHousingAdmin(
  ...
  if (appliError && appliError.code !== "PGRST116")
    throw new Error("App Error: " + appliError.message);

  const userIds = new Set<number>();
  histories?.forEach((h) => userIds.add(h.account_number));
  applications?.forEach(
    (a) => a.student_account_number && userIds.add(a.student_account_number),
  );
  ...
  return users.map((user) => {
    const userHistories =
      histories?.filter((h) => h.account_number === user.account_number) || [];
    const userApps =
      applications?.filter(
        (a) => a.student_account_number === user.account_number,
      ) || [];
    ...
    if (userHistories.length > 0) {
      ...
      const isCurrentlyLivingThere =
        !latestHistory.moveout_date ||
        new Date(latestHistory.moveout_date) > new Date();
    ...
    if (localHousingStatus !== "Assigned" && userApps.length > 0) {
      ...
      currentHousingName =
        userApps[0].housing_name || appRoom?.housing?.[0]?.housing_name || null;
    }
